...
<?php $searchEntry = trim(filter_var($_GET["query"],FILTER_SANITIZE_STRING));
$match = FALSE?>
<?php $aka = "A.K.A. "?>
...
<?php else:?>
    <?php foreach ($superheroes as $element):?> 
        <?php if($searchEntry == $element['alias'] || $searchEntry == $element['name']):?>
            <h3><?= $element['alias'];  ?></h3>
            <h4><?= $aka, $element['name']; ?></h4>
...
<?php if($searchEntry != "" && $match != TRUE):?>
<h3 id = "notFound"> SUPERHERO NOT FOUND! </h3>
<?php endif;?> 
